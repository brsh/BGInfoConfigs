...
#Try {
$Adapter = Get-CIMInstance -Class Win32_NetworkAdapter -filter "Netconnectionstatus=2 or NetconnectionStatus=7"
$Adapter = $Adapter | Sort-Object -Property @{Expression = {$_.NetEnabled}; Ascending = $false}, NetConnectionStatus, NetConnectionID
$Config = Get-CIMInstance -Class Win32_NetworkAdapterConfiguration -Filter "IPEnabled = TRUE"
	...
	$Config | Where-Object { ($_.Description -eq $NICNameRaw) -and ($_.MACAddress -eq $NICMac) } | ForEach-Object {
		if ($_.IPAddress) {
			...
			if ($_.DHCPLeaseExpires) {
				...
				[datetime] $DHCPExpires = $_.DHCPLeaseExpires
			}
